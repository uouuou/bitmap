...
#include "../base/bitmap/UTHashTable_c.h"
#include <assert.h>
...
static int findBitmapInRectAt(MMBitmapRef needle, MMBitmapRef haystack,
        MMPoint *point, MMRect rect, float tolerance, MMPoint startPoint,
	...
	for (scan.y = lastPoint.y; ; --scan.y) {
		for (scan.x = lastPoint.x; ; --scan.x) {
			...
			if (!tableHasKey(jumpTable, color)) {
				addNodeToTable(jumpTable, color, MMPointMake(
										needle->width - scan.x,
